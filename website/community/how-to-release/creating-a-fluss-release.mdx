...
Prepare the two distributions as Docker image inputs before staging moves the
release archives:

```bash
tools $ cd ..
$ RELEASE_VERSION=$RELEASE_VERSION docker/fluss-gateway/prepare_build.sh
$ cd tools
```
...
instead of compiling project sources in buildx. The Dockerfile selects the
matching `build-target/<arch>` directory for each target platform:

```bash
docker/fluss $ cd ../fluss-gateway
docker/fluss-gateway $ docker buildx build \
    --push \
    --platform linux/arm64,linux/amd64 \
    --build-arg FLUSS_VERSION=${RELEASE_VERSION}-rc${RC_NUM} \
