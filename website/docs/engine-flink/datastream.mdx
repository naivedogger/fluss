...
* **setBatch():** Configures batch-read semantics and stops at the latest offsets captured when the source starts
...
Use `setBatch()` for batch reads. The deprecated no-argument `setBounded()` overload is retained for compatibility and is equivalent to `setBatch()`. Use `setBounded(OffsetsInitializer)` for bounded streaming reads.
