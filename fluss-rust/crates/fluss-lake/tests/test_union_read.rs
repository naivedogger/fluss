...
async fn paimon_pk_union_read_merges_update_delete_and_insert_end_to_end() {
    ...
    let scan = lake_table.new_scan().with_batch_size(1);
    let plan = scan
        .plan()
        .await
    ...
    for (name, expected_count) in [("lake-old", 0), ("lake-delete", 0), ("tail-new", 1)] {
        let filtered = lake_table
            .new_scan()
            .with_filter(col("name").eq(name))
            .with_projection(vec![1])
            .with_batch_size(1);
        let filtered_plan = filtered.plan_with_context(&context).await.unwrap();
        let batches = tokio::time::timeout(
            Duration::from_secs(20),
            filtered
                .new_reader()
                .read_splits(filtered_plan.splits())
                .await
                .unwrap()
                .try_collect::<Vec<_>>(),
        )
